...
SELECT department_name, count(department_name) as avg_salary FROM employees JOIN departments USING(department_id) GROUP BY department_name;
...
SELECT department_name, round(avg(salary)) as highest_salary FROM employees JOIN departments USING(department_id) GROUP BY department_name ORDER BY highest_salary DESC LIMIT 1;

SELECT extract(YEAR FROM hire_date) as hired_year, count(*) FROM employees GROUP BY hired_year ORDER BY count DESC LIMIT 1;